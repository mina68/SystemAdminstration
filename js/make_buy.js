...
$(document).ready(function(){
	...
	$.ajax({
		...
		data:{request:'get_next_id'},
		type:'POST',
		success:function(data){
			$('.number-buy h2').text(data);
		}
	})
	...
	$(document).on('click' ,'.save_buy' ,function(event){
		event.preventDefault();
		...
		else
		{
			$.ajax({
				url:'requests/buy.php',
				data:{request:'make_buy' ,notes:buy_notes ,buy_date:buy_date ,seller_name:seller_name ,total_paid:total_paid ,paid:paid},
				type:"POST",
				success:function(data){
					if(!data)
					...
					else
					{
						$('.target_rim').each(function(){
							...
							$.ajax({
								...
								data:{request:'add_rim' ,date_added:date_added,buy_price:buy_price,total_count:total_count ,foreign_buy_id:foreign_buy_id,size:size,holes_number:holes_number,status:status,type:rim_type,evaluative_price:evaluative_price,new_old:new_old,notes:notes},
								type:"POST",
							})
						})
						...
						$('#success-modal .message').text('تم الشراء');
						$('#success-modal').modal('show')
						setTimeout(function(){
							$('#success-modal').modal('hide');
							$('.buy_id').val(data);
